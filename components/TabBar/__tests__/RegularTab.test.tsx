...
describe("RegularTab", () => {
  ...
  describe("rendering", () => {
    ...
    it("should forward testID to the Pressable", () => {
      ...
      const { getByTestId } = render(
        <RegularTab {...buildProps({ testID: "tab-profile" })} />,
      );
      // Assert
    ...
    it("should render different labels for different configs", () => {
      ...
      const { getByText } = render(
        <RegularTab
          {...buildProps({ config: { label: "Perfil", icon: "user" } })}
        />,
      );
      // Assert
  ...
  describe("active dot indicator", () => {
    it("should render the active dot when focused", () => {
      ...
      const { UNSAFE_getAllByType } = render(
        <RegularTab {...buildProps({ isFocused: true })} />,
      );
      const { View } = require("react-native");
      ...
      const activeDot = views.find(
        (v: { props: { className?: string } }) =>
          v.props.className?.includes("bg-primary") &&
    ...
    it("should NOT render the active dot when not focused", () => {
      // Arrange / Act
      const { UNSAFE_getAllByType } = render(
        <RegularTab {...buildProps({ isFocused: false })} />,
      );
      const { View } = require("react-native");
  ...
  describe("press interaction", () => {
    it("should call onPress when pressed", () => {
      ...
      const { getByTestId } = render(
        <RegularTab {...buildProps({ onPress, testID: "tab-home" })} />,
      );
      // Act
