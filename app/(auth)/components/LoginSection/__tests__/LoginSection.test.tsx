...
const mockUseLoginSection = useLoginSection as jest.MockedFunction<
  typeof useLoginSection
>;
...
describe("LoginSection", () => {
  ...
  describe("submit button behavior", () => {
    it("should call onSubmit when the Ingresar button is pressed", () => {
      ...
      mockUseLoginSection.mockReturnValue(
        buildDefaultHookReturn({ onSubmit, isValid: true }),
      );
      const { getByText } = render(
        <LoginSection onSwitchToRegister={jest.fn()} />,
      );
    ...
    it("should disable the submit button when isValid is false", () => {
      // Arrange
      mockUseLoginSection.mockReturnValue(
        buildDefaultHookReturn({ isValid: false, isPending: false }),
      );
      ...
      const submitButton = touchables.find(
        (t: { props: { disabled?: boolean; children: unknown } }) =>
    ...
    it("should enable the submit button when isValid is true", () => {
      // Arrange
      mockUseLoginSection.mockReturnValue(
        buildDefaultHookReturn({ isValid: true, isPending: false }),
      );
      const { UNSAFE_getAllByType } = render(
        <LoginSection onSwitchToRegister={jest.fn()} />,
      );
  ...
  describe("loading state", () => {
    it("should show ActivityIndicator when isPending is true", () => {
      // Arrange
      mockUseLoginSection.mockReturnValue(
        buildDefaultHookReturn({ isPending: true }),
      );
    ...
    it("should not show ActivityIndicator when isPending is false", () => {
      // Arrange
      mockUseLoginSection.mockReturnValue(
        buildDefaultHookReturn({ isPending: false }),
      );
    ...
    it("should disable the button when isPending is true", () => {
      ...
      mockUseLoginSection.mockReturnValue(
        buildDefaultHookReturn({ isPending: true, isValid: true }),
      );
      const { UNSAFE_getAllByType } = render(
        <LoginSection onSwitchToRegister={jest.fn()} />,
      );
  ...
  describe("switch to register", () => {
    it("should call onSwitchToRegister when the Registrarse link is pressed", () => {
      ...
      const { getByText } = render(
        <LoginSection onSwitchToRegister={onSwitchToRegister} />,
      );
