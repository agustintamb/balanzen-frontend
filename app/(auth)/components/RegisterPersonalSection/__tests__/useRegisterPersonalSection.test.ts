...
const buildMockMutate = (
  overrides: Partial<{ mutate: jest.Mock; isPending: boolean }> = {},
) => ({
  mutate: jest.fn(),
  isPending: false,
  ...overrides,
...
const setupMocks = (
  ...
  mockedUseRegistrationStore.mockImplementation(
...
describe("useRegisterPersonalSection", () => {
  ...
  describe("onSubmit — CONSUMIDOR flow", () => {
    it("should call mutate with correct CONSUMIDOR payload when form is submitted", async () => {
      ...
      renderHook(() => useRegisterPersonalSection("CONSUMIDOR", jest.fn()), {
        wrapper: createWrapper(),
      });
      await act(async () => {});
    ...
    it("should call mutate with trimmed firstName and lastName", async () => {
      ...
      mockMutate.mutate.mockImplementation(
        (...args: Parameters<typeof mockMutate.mutate>) => {
    ...
    it("should call persistSession with response data and store setters on success", async () => {
      ...
      mockMutate.mutate.mockImplementation(
        (
          _body: unknown,
          options?: { onSuccess?: (r: RegisterResponse) => void },
        ) => {
  ...
  describe("onSubmit — COMERCIO flow", () => {
    ...
    it("should not navigate to onboarding address for COMERCIO role", async () => {
      ...
      renderHook(() => useRegisterPersonalSection("COMERCIO", jest.fn()), {
        wrapper: createWrapper(),
      });
      await act(async () => {});
  ...
  describe("onSubmit — callback body (mocked handleSubmit)", () => {
      ...
      mockedUseForm.mockImplementationOnce(
        () =>
          ({
            ...
            handleSubmit:
              (fn: (d: typeof VALID_CONSUMIDOR_DATA) => void) => async () =>
                fn(VALID_CONSUMIDOR_DATA),
            trigger: jest.fn(),
            formState: { isValid: true } as ReturnType<
              typeof mockedUseForm
            >["formState"],
    ...
    it("should call persistSession and navigate to onboarding on CONSUMIDOR onSuccess", async () => {
      ...
      let capturedOnSuccess:
        | ((r: RegisterResponse) => Promise<void>)
        | undefined;
      mockMutate.mutate.mockImplementation(
        (
          _: unknown,
          opts?: { onSuccess?: (r: RegisterResponse) => Promise<void> },
        ) => {
