...
describe('useAuthStore', () => {
  ...
  describe('setUser', () => {
    ...
    it('should overwrite an existing user with the new user', () => {
      const firstUser = buildAuthUser({ id: 'user-001', email: 'first@example.com' })
      const secondUser = buildAuthUser({ id: 'user-002', email: 'second@example.com' })
  ...
  describe('setAccessToken', () => {
    it('should update accessToken with the provided token string', () => {
      useAuthStore.getState().setAccessToken('eyJhbGciOiJIUzI1NiJ9.payload.signature')

      expect(useAuthStore.getState().accessToken).toBe('eyJhbGciOiJIUzI1NiJ9.payload.signature')
    })
  ...
  describe('setHasAddress', () => {
    ...
    it('should not affect accessToken or isInitialized when updating has_address', () => {
      const user = buildAuthUser()
      useAuthStore.setState({ user, accessToken: 'token-xyz', isInitialized: true })
  ...
  describe('clear', () => {
    ...
    it('should clear both user and accessToken simultaneously', () => {
      useAuthStore.setState({ user: buildAuthUser(), accessToken: 'active-token' })
    ...
    it('should not modify isInitialized when clearing', () => {
      useAuthStore.setState({ user: buildAuthUser(), accessToken: 'token', isInitialized: true })
